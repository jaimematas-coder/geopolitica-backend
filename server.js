...
async function cicloActualizacion(esRecuperacion) {
  ...
  try {
    ...
    if (lock) { console.log("Ciclo ya en curso, saltando"); return; }
    await redis.set("ciclo_lock", "1");
    await redis.expire("ciclo_lock", 300);
  } catch (e) {
    console.warn("Lock error (continuando): " + e.message);
    // Si falla el lock, continuamos de todas formas
  }
  ...
  try {
    ...
    try { await redis.del("ciclo_lock"); } catch (e) { console.warn("Lock del error: " + e.message); }
  }
}
