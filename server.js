...
async function getHistorialEnviadas() {
  try {
    var keys = await getRedis().keys("enviada_*");
    if (!keys || keys.length === 0) return [];
    var valores = [];
    for (var i = 0; i < keys.length; i++) {
      var val = await getRedis().get(keys[i]);
      if (val) valores.push(typeof val === "string" ? val : JSON.stringify(val));
    }
    return valores.filter(function(v) { return v && v.length > 2; });
  } catch (e) { return []; }
}

async function yaEnviada(titular) {
  limpiarViejos();
  if (idsEnviadasHoy[generarIdNoticia(titular)]) return true;
  try {
    var enRedis = await getRedis().get("enviada_" + generarIdNoticia(titular));
    return !!enRedis;
  } catch (e) { return false; }
}

async function marcarEnviada(titular) {
  var id = generarIdNoticia(titular);
  idsEnviadasHoy[id] = Date.now();
  try {
    await getRedis().set("enviada_" + id, titular.slice(0, 100));
    await getRedis().expire("enviada_" + id, 86400);
  } catch (e) { console.warn("Redis enviada error: " + e.message); }
}
...
async function descargarContenido(url) {
  try {
    ...
    var texto = html
      ...
      .trim();
    if (texto.length < 300) {
      console.warn("Contenido insuficiente en " + url + " — descartando");
      return null;
    }
  ...
  if (noticia.imagen) {
    try {
      await tgReq("sendPhoto", {
        chat_id: chatId, photo: noticia.imagen,
...
async function analizarYEnviar(titulares) {
  ...
  var historialStr = historial.length > 0 ? "\nNoticias YA ENVIADAS en las ultimas 24h (NO repetir estos temas aunque el titular sea diferente):\n" + historial.map(function(h) { return "- " + h; }).join("\n") : "";
  ...
  var sel = await callClaude(
    ...
    "Selecciona SOLO noticias que cumplan estos criterios:\n" +
    "- Tienen consecuencias reales y verificables (accion militar, acuerdo firmado, decision politica de impacto, escalada diplomatica concreta)\n" +
    "- NO son declaraciones, opiniones, rumores ni noticias de 'X ha dicho que Y'\n" +
    "- NO son noticias de relleno o seguimiento rutinario sin novedad real\n" +
    "- NO repetir el MISMO ACONTECIMIENTO CONCRETO que ya fue enviado (mismo hecho, mismo dia, mismos actores, mismo lugar). Una actualizacion con datos nuevos (nuevo precio, nueva base atacada, nuevo desarrollo) SI es una noticia distinta aunque sea el mismo tema general.\n" +
    "- MENA: puntuacion minima 7. Global: puntuacion minima 9. Si no llegan, devuelve array vacio.\n" +
    "- Maximo 3 noticias. Si no hay nada nuevo y relevante, devuelve array vacio.\n" +
    "- CRITICO: incluye en 'fuentes' UNICAMENTE titulares de esta lista que traten EXACTAMENTE el mismo hecho.\n" +
    ...
    "{\"seleccion\":[{\"titulo_original\":\"titulo exacto\",\"url\":\"url del articulo\",\"puntuacion\":8,\"region\":\"MENA\",\"categoria\":\"Conflictos armados\",\"seguimiento\":\"\",\"fuentes\":[{\"medio\":\"nombre\",\"url\":\"url exacta\"}]}]}"
  );

  var seleccionadas = [];
  for (var i = 0; i < (sel.seleccion || []).length; i++) {
    var s = sel.seleccion[i];
    var enviada = await yaEnviada(s.titulo_original || "");
    if (!enviada) seleccionadas.push(s);
  }
  ...
  for (var i = 0; i < seleccionadas.length; i++) {
    ...
    articulosConContenido.push({
      ...
      fuentes: s.fuentes, puntuacion: s.puntuacion, contenido: contenido,
    });
  }
  ...
  var red = await callClaude(
    "Eres analista geopolitico senior especializado en MENA. JSON valido unicamente.",
    "Redacta estas noticias para un canal de divulgacion geopolitica.\n" +
    "NORMAS ESTRICTAS:\n" +
    "- Solo hechos que aparezcan explicitamente en el contenido del articulo.\n" +
    "- Si citas un dato, debe estar literalmente en el contenido.\n" +
    "- Las fuentes son exactamente las proporcionadas. No inventes otras.\n" +
    "- Bloque extra: SOLO si el contenido aporta informacion concreta adicional. Maximo 3 puntos, una frase cada uno. Si no hay suficiente, deja vacio.\n" +
    "- Tipos de bloque extra: claves, analisis, actores, contexto.\n\n" +
    "FORMATO DEL CAMPO 'texto' (para canal de difusion publico):\n" +
    "- Sin titular. Solo 2-3 frases directas explicando el hecho y su relevancia.\n" +
    "- Tono neutro, informativo, sin valoraciones. Breve y sin relleno.\n\n" +
    "CAMPO 'banderas': emojis de banderas de los paises protagonistas (max 3). Solo paises directamente implicados.\n" +
    "CAMPO 'titular': titular breve en espanol para uso interno.\n\n" +
    "Articulos:\n" + articulosStr + "\n\n" +
    "{\"noticias\":[{\"titulo_original\":\"exacto\",\"titular\":\"espanol breve\",\"banderas\":\"🇮🇷 🇺🇸\",\"texto\":\"2-3 frases directas sin titular\",\"analisis\":\"implicaciones basadas en el contenido\",\"triangulacion\":{\"fuentes\":[\"medio1\"],\"contradicciones\":\"\"},\"extra\":{\"tipo\":\"claves\",\"contenido\":\"\"}}]}"
  );
...
async function procesarComando(texto) {
  ...
  if (cmd === "/estado") {
    ...
    if (!ok) await tgTexto("Error al suspender. Intentalo desde Render.");
