...
async function expandirNoticia(noticia, chatId, msgId, nid) {
  ...
  // Reemplazar el mensaje miniatura con el contenido completo (interno + externo juntos)
  var textoCompleto = textos.interno + "\n\n" + textos.externo;

  // Telegram limita caption y mensaje a 4096 chars, dividir si hace falta
  if (textoCompleto.length > 4000) {
    // Editar mensaje con interno
    ...
    // Enviar externo como mensaje encadenado
    await tgReq("sendMessage", {
      chat_id: chatId,
      text: textos.externo,
      parse_mode: "Markdown",
  ...
  // Enviar imagen si existe (como mensaje separado encadenado)
  if (noticia.imagen) {
    try {
...
async function analizarYEnviar(titulares) {
  ...
  var red = await callClaude(
    ...
    "- claves, analisis, actores, contexto\n\n" +
    "Noticias:\n" + titulosStr + "\n\n" +
