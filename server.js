...
async function procesarComando(texto) {
  ...
  if (cmd === "/estado") {
    ...
    var lineas = [];
    lineas.push("URL definida: " + (process.env.UPSTASH_REDIS_REST_URL ? "SI" : "NO"));
    lineas.push("TOKEN definido: " + (process.env.UPSTASH_REDIS_REST_TOKEN ? "SI" : "NO"));
    try {
      ...
      if (typeof leido === "string") { try { leido = JSON.parse(leido); } catch (e) {} }
      var ok = leido && leido.marca === marca;
      lineas.push("Escritura/lectura: " + (ok ? "OK" : "FALLA"));
      lineas.push("Tipo devuelto: " + typeof leido);
      if (!ok) lineas.push("Valor: " + String(JSON.stringify(leido)).slice(0, 120));
    } catch (e) {
      lineas.push("ERROR Redis: " + e.message);
    }
    try {
      ...
      var ttl = await getRedis().ttl("historial_noticias");
      lineas.push("TTL: " + ttl + "s");
    } catch (e) {
      lineas.push("ERROR leyendo historial: " + e.message);
    }
