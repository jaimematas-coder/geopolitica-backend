...
// Almacen de noticias pendientes de expansion (para botones)
var noticiasCache = {};
  ...
  if (body.callback_query) {
    ...
    if (cbData.startsWith("ver_")) {
      ...
      if (noticia) {
        ...
        await telegramRequest("sendMessage", { chat_id: cbChatId, text: "La noticia ya no esta disponible en cache." });
      }
...
// ── Init ──────────────────────────────────────────────────────────────────────
async function init() {
  // Esperar 3 segundos para que la red este lista
  await new Promise(function(r) { setTimeout(r, 3000); });
  try { await redis.del("ciclo_lock"); } catch (e) { console.warn("Lock clear error: " + e.message); }
  var apagado = await getApagado();
  if (!apagado) {
