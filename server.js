...
async function analizarYEnviar(titulares, esRecuperacion) {
  ...
  var resumen = titulares.slice(0, 35).map(function(t) {
    return "- [" + t.medio + "] " + t.titulo + (t.imagen ? " [IMG]" : "") + " | URL: " + t.link;
  }).join("\n");

  // LLAMADA 1: seleccion y puntuacion
  console.log("Claude paso 1: seleccionando noticias...");
  var seleccion = await callClaude(
    ...
    "Titulares recientes (ultimas 48h):\n" + resumen + "\n\n" +
    "Selecciona entre 3 y 6 noticias verdaderamente importantes. Si no hay nada relevante, devuelve array vacio. Umbrales: MENA >= 6, Global >= 8. Si varios titulares tratan el mismo hecho, cuentalos como UNO y elige la URL mas especifica.\n" +
    "Categorias validas UNICAMENTE: Conflictos armados, Diplomacia, Seguridad y defensa, Economia global, Energia y recursos, Derechos humanos.\n" +
    "Regiones validas: MENA, Europa, Asia-Pacifico, America, Africa Subsahariana, Global.\n\n" +
    "{\"seleccion\":[{\"titulo_original\":\"Titulo exacto\",\"url_noticia\":\"URL especifica del articulo\",\"puntuacion\":8,\"region\":\"MENA\",\"categoria_tematica\":\"Conflictos armados\",\"seguimiento\":\"nombre o cadena vacia\",\"fuentes_detectadas\":[{\"medio\":\"Al Jazeera\",\"url\":\"URL especifica\"}]}]}"
  );

  var seleccionadas = seleccion.seleccion || [];
  if (seleccionadas.length === 0) {
    console.log("No hay noticias relevantes en este ciclo");
    return [];
  }
  ...
  var titulosSeleccionados = seleccionadas.map(function(s) {
    return "- " + s.titulo_original + " [" + s.region + "] [" + s.categoria_tematica + "]";
  }).join("\n");

  var redaccion = await callClaude(
    "Eres analista geopolitico senior especializado en MENA. Responde UNICAMENTE con JSON valido. Sin markdown.",
    "Redacta estas noticias para un canal de divulgacion geopolitica en Instagram.\n" +
    "NORMAS ESTRICTAS DE TONO:\n" +
    "- Solo hechos verificables y sus consecuencias objetivas. Cero valoraciones morales o ideologicas.\n" +
    "- Si se menciona un dato (porcentaje, cifra, nombre de acuerdo), indicar entre parentesis la fuente: (segun ONU), (segun Reuters), etc.\n" +
    "- El analisis interno debe explicar que implica el hecho geopoliticamente, no juzgar si es bueno o malo.\n" +
    "- Categorias tematicas validas UNICAMENTE: Conflictos armados, Diplomacia, Seguridad y defensa, Economia global, Energia y recursos, Derechos humanos.\n\n" +
    ...
    "\"extra\":{\"tipo\":\"claves\",\"contenido\":\"Solo si aporta informacion factual adicional relevante. Vacio si no aplica.\"}" +
    "}]}"
  );

  // Combinar seleccion + redaccion
  ...
  var noticias = noticiasRedactadas.map(function(n) {
    ...
    // Usar URLs reales de la seleccion
    var fuentes_links = sel.fuentes_detectadas || [];

    // Triangulacion
    var triangulacion = {
      fuentes_contrastadas: fuentes_links.map(function(f) { return f.medio || f; }),
      contradicciones: ""
    };
