...
async function descargarContenido(url) {
  try {
    ...
    var html = res.data || "";
    // Extraer texto relevante eliminando HTML
    var texto = html
      ...
      .replace(/\s+/g, " ")
      .trim();
    // Limitar a 3000 caracteres del cuerpo principal
...
async function analizarYEnviar(titulares) {
  ...
  for (var i = 0; i < seleccionadas.length; i++) {
    ...
    articulosConContenido.push({
      ...
      contenido: contenido || "[Contenido no disponible - posible paywall]",
    });
  }

  console.log("Claude paso 2: redaccion con contenido real...");
