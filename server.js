...
// ── Descarga de contenido de articulos ───────────────────────────────────────
async function descargarContenido(url) {
  try {
    ...
    // Si hay muy poco texto, es paywall, login o pagina de error
    if (texto.length < 300) {
      console.warn("Contenido insuficiente en " + url + " (" + texto.length + " chars) — descartando");
      return null;
    }
...
async function analizarYEnviar(titulares) {
  ...
  var sel = await callClaude(
    ...
    "- NO son declaraciones, opiniones, rumores, filtraciones no confirmadas ni noticias de 'X ha dicho que Y'\n" +
    "- NO son noticias de relleno, contexto general o seguimiento rutinario sin novedad real\n" +
    "- MENA: puntuacion minima 7 (no 6). Global: puntuacion minima 9. Si no llegan al umbral, devuelve array vacio.\n" +
    "- Maximo 3 noticias. Si no hay nada que cumpla estos criterios con claridad, devuelve array vacio.\n" +
    "CRITICO: Para cada noticia, incluye en 'fuentes' UNICAMENTE los titulares de esta lista que traten EXACTAMENTE el mismo hecho.\n" +
    ...
    "{\"seleccion\":[{\"titulo_original\":\"titulo exacto del titular seleccionado\",\"url\":\"url exacta de ese titular\",\"puntuacion\":8,\"region\":\"MENA\",\"categoria\":\"Conflictos armados\",\"seguimiento\":\"\",\"fuentes\":[{\"medio\":\"nombre del medio\",\"url\":\"url exacta del titular de ese medio que cubre el MISMO hecho\"}]}]}"
  );
  ...
  console.log("Descargando contenido de articulos seleccionados...");
  ...
  for (var i = 0; i < seleccionadas.length; i++) {
    ...
    if (!contenido) {
      console.warn("Articulo descartado (sin acceso): " + s.titulo_original);
      continue; // Descartar articulos sin contenido accesible
    }
  ...
  if (articulosConContenido.length === 0) {
    console.log("Todos los articulos tienen paywall o son inaccesibles");
    return;
  }
  console.log("Articulos con contenido accesible: " + articulosConContenido.length);
  ...
  var red = await callClaude(
    ...
    "NORMAS ESTRICTAS:\n" +
    "- Solo hechos que aparezcan explicitamente en el contenido del articulo. Cero inferencias ni conocimiento externo.\n" +
    "- Si citas un dato (cifra, nombre, acuerdo), debe estar literalmente en el contenido del articulo.\n" +
    "- Las fuentes son exactamente las proporcionadas. No añadas ni inventes otras.\n" +
    "- CRITICO SOBRE FUENTES: No inventes ni añadas fuentes. No cites datos de fuentes no proporcionadas.\n" +
    "- Bloque extra: SOLO si el contenido del articulo aporta informacion concreta adicional. Maximo 3 puntos, una frase cada uno. Si no hay suficiente, deja vacio.\n" +
    "- Tipos de bloque extra: claves, analisis, actores, contexto.\n\n" +
    "FORMATO DEL CAMPO 'texto' (para canal de difusion):\n" +
    "- Sin titular. Solo 2-3 frases directas explicando el hecho y su relevancia.\n" +
    "- Tono neutro, informativo, sin valoraciones.\n" +
    "- Maximo 3 frases. Breve y sin relleno.\n\n" +
    "CAMPO 'banderas': emojis de banderas de los paises protagonistas de la noticia (max 3 emojis). Solo paises directamente implicados.\n\n" +
    "Articulos:\n" + articulosStr + "\n\n" +
    "{\"noticias\":[{\"titulo_original\":\"exacto\",\"titular\":\"espanol\",\"banderas\":\"🇮🇷 🇺🇸\",\"texto\":\"2-3 frases directas sin titular\",\"analisis\":\"implicaciones geopoliticas basadas en el contenido\",\"triangulacion\":{\"fuentes\":[\"medio1\"],\"contradicciones\":\"\"},\"extra\":{\"tipo\":\"claves\",\"contenido\":\"\"}}]}"
  );
  ...
  for (var i = 0; i < redactadas.length; i++) {
    ...
    var s = articulosConContenido.find(function(x) {
      return x.titulo && n.titulo_original &&
        x.titulo.slice(0, 20) === n.titulo_original.slice(0, 20);
    }) || {};
