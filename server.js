...
async function analizarYEnviar(titulares, esRecuperacion) {
  ...
  // LLAMADA 1: solo seleccion y puntuacion (rapida)
  ...
  var seleccion = await callClaude(
    ...
    "Titulares:\n" + resumen + "\n\n" +
    "Selecciona las 5 noticias mas relevantes. Umbrales: MENA >= 6, Global >= 8. Si varios titulares tratan el mismo hecho, cuentalos como UNO. Prioriza los marcados con [IMG].\n" +
    "{\"seleccion\":[{\"titulo_original\":\"Titulo exacto\",\"puntuacion\":8,\"region\":\"MENA\",\"categoria_tematica\":\"Conflictos armados\",\"seguimiento\":\"nombre o cadena vacia\",\"fuentes_detectadas\":[\"Medio1\",\"Medio2\"]}]}"
  );

  var seleccionadas = seleccion.seleccion || [];
  if (seleccionadas.length === 0) { console.log("No hay noticias relevantes"); return []; }
  console.log("Seleccionadas: " + seleccionadas.length);

  // LLAMADA 2: redaccion de cada noticia seleccionada (con contexto reducido)
  console.log("Claude paso 2: redactando noticias...");
  var titulosSeleccionados = seleccionadas.map(function(s) { return "- " + s.titulo_original + " [" + s.region + "] [" + s.categoria_tematica + "]"; }).join("\n");
  var redaccion = await callClaude(
    "Eres analista geopolitico senior especializado en MENA. Redacta noticias para Instagram con criterio propio. Responde UNICAMENTE con JSON valido. Sin markdown.",
    "Redacta estas noticias para un canal de divulgacion geopolitica en Instagram:\n" + titulosSeleccionados + "\n\n" +
    "Para cada una: texto de 2-3 frases (informativo o analitico segun requiera), analisis interno breve, triangulacion de fuentes si las hay, y decide si necesita bloque extra (no siempre): claves, implicaciones, visita_estado, reaccion_potencias.\n\n" +
    "{\"noticias\":[{" +
    "\"titular\":\"Titular breve en espanol\"," +
    "\"texto_instagram\":\"2-3 frases. Sin hashtags.\"," +
    "\"analisis_interno\":\"Por que importa y angulo elegido.\"," +
